...
POWER_CONTROL_1 = const(0xC0)
POWER_CONTROL_2 = const(0xC1)
VCOM_CONTROL_1 = const(0xC5)
VCOM_CONTROL_2 = const(0xC7)
FRAME_RATE_CONTROL = const(0xB2)
DISPLAY_FUNCTION_CONTROL = const(0xB6)
...
class ILI9488:
    ...
    def Init(self):
        for command, data in (
            (READ_DISPLAY, b"\x03\x80\x02"),
            (POWER_CONTROL_B, b"\x00\xc1\x30"),
            (POWER_ON_CONTROL, b"\x64\x03\x12\x81"),
            (DRIVER_TIMING_CONTROL_A, b"\x85\x00\x78"),
            (POWER_CONTROL_A, b"\x39\x2c\x00\x34\x02"),
            (PUMP_RATIO_CONTROL, b"\x20"),
            (DRIVER_TIMING_CONTROL_B, b"\x00\x00"),
            (POWER_CONTROL_1, b"\x23"),
            (POWER_CONTROL_2, b"\x10"),
            (VCOM_CONTROL_1, b"\x3e\x28"),
            (VCOM_CONTROL_2, b"\x86"),
        ):
        ...
        if self.rotation == 0:  # 0 deg
            ...
            self.WriteDevice(MEMORY_ACCESS_CONTROL, b"\x08")
        for command, data in (
            (PIXEL_FORMAT_SET, b"\x55"),
            (FRAME_RATE_CONTROL, b"\x00\x18"),
            (DISPLAY_FUNCTION_CONTROL, b"\x02\x02\x3B"),
            (ENABLE_3G, b"\x00"),
            (GAMMA_SET, b"\x01"),
            (
                POS_GAMMA_CONTROL,
    ...
    def FillRectangle(self, x, y, w, h, color=None):
        ...
        for i in range(MEMORY_BUFFER):
            self.buffer[2 * i] = color[0]
            self.buffer[2 * i + 1] = color[1]
...
TFT_CLK_PIN, TFT_MOSI_PIN, TFT_MISO_PIN, TFT_CS_PIN = (
    const(10),
    const(11),
    const(12),
    const(9),
    ...
    for x in range(len(bitmap_image_files)):
        ...
        if f.read(2) == b"BM":  # header
            ...
            if (width * height * 2) > MEM_SIZE:
                no_of_read_buffer, balance_memory_size = (
                    width * height * 2
                ) / MEM_SIZE, (width * height * 2) % MEM_SIZE
            else:
                no_of_read_buffer = 0
